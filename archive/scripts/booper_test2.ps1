# Booper press test at Z=-14. HMI stays powered on after test.
...
function GrabTop([string]$label) {
    $path = "C:\Claude\hmi430\test2_${label}_top.jpg"
    $sf = Await ([Windows.Storage.StorageFolder]::GetFolderFromPathAsync("C:\Claude")) ([Windows.Storage.StorageFolder])
    $f  = Await ($sf.CreateFileAsync("test2_${label}_top.jpg", [Windows.Storage.CreationCollisionOption]::ReplaceExisting)) ([Windows.Storage.StorageFile])
    ...
    Write-Host "  top:   test2_${label}_top.jpg ($([int](Get-Item $path).Length/1024) KB)"
}

function GrabFront([string]$label) {
    $path = "C:\Claude\hmi430\test2_${label}_front.jpg"
    $sf = Await ([Windows.Storage.StorageFolder]::GetFolderFromPathAsync("C:\Claude")) ([Windows.Storage.StorageFolder])
    $f  = Await ($sf.CreateFileAsync("test2_${label}_front.jpg", [Windows.Storage.CreationCollisionOption]::ReplaceExisting)) ([Windows.Storage.StorageFile])
    Await ($mcFront.CapturePhotoToStorageFileAsync(
        [Windows.Media.MediaProperties.ImageEncodingProperties]::CreateJpeg(), $f)) $null
    Write-Host "  front: test2_${label}_front.jpg ($([int](Get-Item $path).Length/1024) KB)"
}
...
# Lift back to view position
Write-Host "`n=== Lifting to Z=0 ==="
Send $port 'G0 Z0' -wait
Start-Sleep -Milliseconds 1000
GrabTop  "after"
GrabFront "after"

# Stay at view position, HMI stays on — do NOT send M5
Write-Host "`nHMI left powered on. Machine at X=$XPos Y=$YPos Z=0."
Write-Host "Check test2_pressed_top.jpg for screen response."
$port.Close()
$mcTop.Dispose()
$mcFront.Dispose()
