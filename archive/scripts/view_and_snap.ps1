# Move bed to view position, capture screen
...
# Connect CNC and move to view position
...
$f  = Await ($sf.CreateFileAsync("latest.jpg", [Windows.Storage.CreationCollisionOption]::ReplaceExisting)) ([Windows.Storage.StorageFile])
Await ($mc.CapturePhotoToStorageFileAsync(
    [Windows.Media.MediaProperties.ImageEncodingProperties]::CreateJpeg(), $f)) $null
$img = [System.Drawing.Image]::FromFile("$outDir\latest.jpg")
$img.RotateFlip([System.Drawing.RotateFlipType]::Rotate180FlipNone)
$img.Save("$outDir\latest.jpg"); $img.Dispose()
Write-Host "Saved: $outDir\latest.jpg"

$port.Close(); $mc.Dispose()
